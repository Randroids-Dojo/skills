...
For articulated meshes, include the actual conversion into the moving parent's coordinates and back. Small floating-point displacement can make nearly collinear cut fragments intersect even when the original mesh passes. Retain both representations and map any shell-local triangle witnesses back to source indices before editing. Resolve unnecessary cuts or inconsistent shared boundaries within the physical interface; a small transform-error number alone does not establish valid output geometry.
...
An incomplete conservative coverage proof is neither a demonstrated collision nor a clean result. Retain its exact residuals before changing geometry. Where a residual crosses several finite mating triangles, partition against their actual union and verify complete coverage at the original tolerance. For a convex target, a convex cell whose vertices all satisfy the same-target distance bound satisfies that bound throughout; vertices supported by different targets do not provide that guarantee. Independently replay the partition, retain every remainder, and include interior gaps, missing targets, altered cells and exhausted budgets as rejection cases. Use this method when finite coverage is required, without turning every visual inspection into an analytical proof.
...
Separate a simplifier's local plane-error bound, native normal-encoding accuracy and preservation of the original surface field. A closed reduced mesh can encode its new target normals accurately while substantially changing the original shading between vertices. Compare geometry, UVs and interpolated native normals against corresponding finite physical sheets, including triangle interiors; avoid accidentally matching a nearby opposing wall. Use the actual normal discontinuities and manufactured returns to define protected regions, rather than flags introduced only by a normal codec. Reject a poor or ineffective candidate before spending on full captures, and judge any permitted field change at its intended viewing distance.
