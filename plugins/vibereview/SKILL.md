---
name: vibereview
description: Use the VibeReview macOS CLI to run browser-game playtest review sessions, capture screenshot/browser-state evidence, and feed findings into Spiral-HTML ledgers such as PLAYTEST.html, FOLLOWUPS.html, OPEN_QUESTIONS.html, and FUN_FACTOR_AUDIT.html.
compatibility: Requires macOS with VibeReview built or installed. Browser-state snapshots require the VibeReview Chrome extension and port 37717 free.
---

# VibeReview

VibeReview is the Mac-native playtest capture tool for vibe-coded browser games. The CLI records review sessions, screenshots, browser-state snapshots, notes, severity, rating, and tags, then writes review artifacts into the target project. In Spiral-HTML projects, those artifacts are evidence for the qualitative gate: `docs/PLAYTEST.html`, `docs/FOLLOWUPS.html`, `docs/OPEN_QUESTIONS.html`, and `docs/FUN_FACTOR_AUDIT.html`.

Use this skill when the user asks to review, playtest, capture feedback, record evidence, or turn a human game-review observation into project artifacts.
...
- `spiral-html` owns scaffold bootstrap and drift audit. Use it when the project needs `init` or `audit`; do not recreate the full scaffold by hand inside this skill.
...
- Use short, concrete capture notes. Put one observation per capture.
- Prefer tags that identify the subsystem or feel axis: `controls`, `feel`, `onboarding`, `performance`, `audio`, `difficulty`, `accessibility`.
- Use `--note-file -` for multiline notes:

  ```bash
  printf '%s\n' "Boss intro is visually clear, but the first attack has no readable tell." \
    | vibereview capture --project /path/to/game --note-file - --severity blocksRelease --rating 2 --tags boss,readability
  ```

- If the user asks for a review summary, summarize captures by severity, affected system, artifact links, and suggested Spiral-HTML followups.
