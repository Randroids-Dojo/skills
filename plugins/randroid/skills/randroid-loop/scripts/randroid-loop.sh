...
if [[ -z "$GIT_WORKFLOW" ]]; then
    ...
    echo ""
    echo "  1) commit   - Commit locally only, no push (default)"
    echo "  2) none     - Leave verified changes uncommitted"
    echo "  3) push     - Commit and push to current branch"
    echo "  4) pr       - Open PR, wait for CI (no merge)"
    echo "  5) pr-merge - Open PR, wait for CI, then auto-merge"
    echo ""
    read -p "Select [1-5, default=1]: " workflow_choice
    ...
    case "$workflow_choice" in
        ...
        5|pr-merge|merge)
            ...
            ;;
        *)
            echo "Invalid selection. Using 'commit'."
            GIT_WORKFLOW="commit"
            ;;
    esac
fi
...
# Select one durable task format before initializing state.
TASK_TRACKER=""
if [[ -d ".dots" ]] && find .dots -name '*.html' -print -quit 2>/dev/null | grep -q . && find .dots -name '*.md' -print -quit 2>/dev/null | grep -q .; then
    echo "Error: .dots contains both Markdown and HTML task files. Resolve the tracker conflict before starting the loop."
    exit 1
elif [[ -d ".dots" ]] && find .dots -name '*.html' -print -quit 2>/dev/null | grep -q .; then
    TASK_TRACKER="dot-html"
elif [[ -d ".dots" ]] && find .dots -name '*.md' -print -quit 2>/dev/null | grep -q .; then
    TASK_TRACKER="dot"
elif command -v dot-html >/dev/null 2>&1 && command -v dot >/dev/null 2>&1; then
    if [[ ! -t 0 ]]; then
        echo "Error: Both dot and dot-html are available. Run interactively to select a tracker or initialize .dots first."
        exit 1
    fi
    echo ""
    echo "Which task tracker should this project initialize?"
    echo "  1) dot      - Markdown-backed Dots"
    echo "  2) dot-html - HTML-backed Dots"
    read -p "Select [1-2]: " tracker_choice
    case "$tracker_choice" in
        1|dot) TASK_TRACKER="dot" ;;
        2|dot-html) TASK_TRACKER="dot-html" ;;
        *) echo "Invalid task tracker selection."; exit 1 ;;
    esac
...
if [[ -z "$ITERATIONS" ]]; then
    ...
    echo ""
    echo "  inf) Infinite - Loop forever (Ctrl+C to stop, explicit only)"
    echo " comp) Until complete - Stop when tasks exhausted (default)"
    echo "    #) Or enter a custom number"
    echo ""
    read -p "Select [inf/comp/#, default=comp]: " iter_choice
    ...
    case "$iter_choice" in
        ...
        *)
            if [[ "$iter_choice" =~ ^[0-9]+$ ]]; then
                ...
                echo "Invalid selection. Using until-complete mode."
                ITERATIONS=0
            fi
            ;;
    esac
...
if [[ -n "$DIRECTIONS" ]]; then
    "${SCRIPT_DIR}/setup-loop.sh" "$MODE" --iterations "$ITERATIONS" --git-workflow "$GIT_WORKFLOW" --task-tracker "$TASK_TRACKER" --fresh-context --directions "$DIRECTIONS"
else
    "${SCRIPT_DIR}/setup-loop.sh" "$MODE" --iterations "$ITERATIONS" --git-workflow "$GIT_WORKFLOW" --task-tracker "$TASK_TRACKER" --fresh-context
fi

# Prompt files
PROMPT_FILE="${SKILL_DIR}/references/${MODE}-loop.md"
SHARED_FILE="${SKILL_DIR}/references/loop-shared.md"
