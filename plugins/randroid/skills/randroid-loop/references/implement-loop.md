...
```bash
<dot-cli> show <task-id>  # Look for ## Verify section with checklist items
```
...
Complete the task:
```bash
<dot-cli> off <task-id> -r "brief summary"
```

Then proceed to **step 4** for the explicitly selected git workflow.
...
- Add focused tests as scoped changes; let the shared git workflow decide whether they are committed
...
- Run `<dot-cli> tree` to see the dependency graph
...
```bash
<dot-cli> add "implement: <improvement>" -d "Details..."
<dot-cli> add "research: <question>" -d "Need to investigate..."
```
...
```bash
<dot-cli> off <task-id> -r "Build passes, tests pass, verified feature X works manually"
```
...
- **One task per iteration**: When commits are authorized, keep one task per commit
